const { UsedResource } = require('./UsedResource');
...
class Pool {
  ...
  release(resource) {
    for (let i = 0, l = this.used.length; i < l; ++i) {
      ...
      if (used.resource === resource) {
        ...
        this.free.push(new FreeResource(used.resolve()));
  ...
  _doAcquire() {
    ...
    while (this._shouldAcquire()) {
      ...
      this.free.pop();
      this.used.push(new UsedResource(free.resolve()));
  ...
  _create() {
    ...
    callbackOrPromise(this.creator)
      .then(resource => {
        ...
        this.free.push(new FreeResource(resource));
